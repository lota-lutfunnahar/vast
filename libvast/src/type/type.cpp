...
type::type(std::span<const std::byte> table, const chunk_ptr& context) noexcept
  : table_{context->slice(table)} {
  // nop
}

type::type(std::span<const std::byte> table, const type& context) noexcept
...
bool operator==(const type& lhs, const type& rhs) noexcept {
  const auto lhs_view = as_bytes(lhs);
  const auto rhs_view = as_bytes(rhs);
...
bool operator<(const type& lhs, const type& rhs) noexcept {
  const auto lhs_view = as_bytes(lhs);
  const auto rhs_view = as_bytes(rhs);
  return std::lexicographical_compare(lhs_view.begin(), lhs_view.end(),
...
/// identical.
std::span<const std::byte> as_bytes(const class type& type) {
