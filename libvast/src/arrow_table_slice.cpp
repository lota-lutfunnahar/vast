...
template <>
struct decodable<duration_type, arrow::Int64Array> : std::true_type {};
...
auto legacy_duration_at(const arrow::Int64Array& arr, int64_t row) {
...
public:
  ...
  void operator()(const arrow::NumericArray<T>& arr, const U&) {
    ...
    if constexpr (detail::is_any_v<U, real_type, integer_type, count_type,
      ...
      result_ = legacy_duration_at(arr, row_);
    }
  }
...
public:
  ...
  void operator()(const arrow::NumericArray<arrow::Int64Type>& arr,
                  const duration_type&) {
    apply(arr, legacy_duration_at);
  }
