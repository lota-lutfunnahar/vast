...
void xxhash64::add(std::span<const std::byte> bytes) noexcept {
  auto state = reinterpret_cast<XXH64_state_t*>(&state_);
  auto result = ::XXH64_update(state, &*bytes.begin(), bytes.size());
  VAST_ASSERT(result == XXH_OK);
}

auto xxhash64::finish() noexcept -> result_type {
  auto state = reinterpret_cast<XXH64_state_t*>(&state_);
