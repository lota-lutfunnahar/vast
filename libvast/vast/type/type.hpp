...
public:
  ...
  type(std::span<const std::byte> table, const chunk_ptr& context) noexcept;
  ...
  type(std::span<const std::byte> table, const type& context) noexcept;
  ...
  /// identical.
  friend std::span<const std::byte> as_bytes(const class type& type);
...
private:
  ...
  make(flatbuffers::FlatBufferBuilder& builder,
       flatbuffers::Offset<concrete_table_type> type_offset,
       flatbuffers::Offset<
         flatbuffers::Vector<flatbuffers::Offset<fbs::type::Attribute>>>
         attribute_offset
       = {}) {
    builder.Finish(fbs::CreateType(builder, concrete_type_index,
                                   type_offset.Union(), attribute_offset));
...
template <>
struct equal_to<vast::experimental::type> : hash<std::span<const std::byte>> {
